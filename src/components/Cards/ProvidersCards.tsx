...
import { fetchMovies } from "@/src/api/tmdb";
import { Ionicons } from "@expo/vector-icons";
import { Provider } from "@/src/types/interfaces";
...
export default function ProvidersCards() {
  ...
  useEffect(() => {
    async function loadProviders() {
      try {
        ...
        setProviders(data.results.slice(0, 20) ?? []);
      } catch {
  ...
  return (
    <FlatList
      ...
      ListFooterComponent={
        <TouchableOpacity
          onPress={() => router.push(`/Provider/Providers` as any)}
        >
...
function ProviderCard({ item }: { item: Provider }) {
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.7}
      onPress={() => router.push(`/Provider/${item.provider_id}` as any)}
    >

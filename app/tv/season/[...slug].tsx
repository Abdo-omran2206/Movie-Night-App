...
import { fetchTvSeasonDetails, getTvById } from "@/src/api/tmdb";
...
import StreamModel from "@/src/components/Models/StreamModel";
...
import { encodeId } from "@/src/lib/hash";
...
export default function SeasonDetailsScreen() {
  ...
  const fallbackSvg = React.useMemo(() => {
    if (!season) return "";
    return generateMovieAvatar(season.name || `Season ${season.season_number}`);
  }, [season]);
